...
\end{vidu}
\loai{Xác định cường độ tức thời tại thời điểm t}
\begin{vidu}%[3P3-2.2Y][Loai5]
	Cường độ dòng điện qua một đoạn mạch có biểu thức $i=5\sqrt{2}\cos 100\pi t$ A (t tính bằng s). Cường độ dòng điện tức thời tại thời điểm $t = 2023$ s là
	\choice
	{$-5\sqrt{2}$ A}
	{5 A}
	{\True $5\sqrt{2}$ A}
	{$-5$ A}
	\loigiai{
		Tại $t = 2023$ s: $\phi_{2015\ s}\equiv 0$ $\Rightarrow$ $i={I_0}=5\sqrt{2}$ A.
	}
\end{vidu}
\loai{Xác định điện áp tức thời tại thời điểm t}
...
\begin{vidu}%[3P3-2.2K][Loai7]
	Dòng điện xoay chiều qua một đoạn mạch có biểu thức $i={I_0}\cos \left( 120\pi t-\dfrac{\pi}{3} \right)\ A$. Thời điểm thứ 2023 độ lớn cường độ dòng điện bằng cường độ dòng điện hiệu dụng là
	\choice
	{8,15 s}
	{\True 8,43 s}
	{9,26 s}
...
%	{Có thể dùng VTLG cho dễ hình dung}
		\begin{itemize}	
			\centerline{\begin{tikzpicture}[scale=1,thick,>=stealth']
				...
				\end{tikzpicture}}			
			\item Tách $2023 = 2020 + 3$.
			Thời điểm cần tìm là: ${{t}}=505T+\dfrac{T}{6}+\dfrac{T}{4}+\dfrac{T}{8}\approx 8,43\ s$.
	\end{itemize}
}
\end{vidu}
...
\end{vidu}
\loai{Xác định số lần đạt u, i trong thời gian cho trước}
\begin{vidu} %[3P3-2.2B][Loai9] 
	Điện áp giữa hai đầu một đoạn mạch là $u=220\sqrt{2}\cos \left( 100\pi t \right)\ V$. Cứ mỗi giây số lần điện áp này bằng 0 là
	\choice
	{\True 100 lần}
	{50 lần}
	{200 lần}
...
%		\Binhluan{
		\begin{itemize}
			\item Mỗi dao động toàn phần có 2 thời điểm $ u=0$. Vậy mỗi giây dòng điện có 2f lần bằng 0.
			\item Vậy với $2f=2.\dfrac{\omega }{2\pi }=50.2=100$ lần.
		\end{itemize}
%}
%{Trong mỗi giây dòng điện tương ứng cũng đổi chiều 2f lần}
	}
\end{vidu}
